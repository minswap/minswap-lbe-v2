...
import { AuthenMintingPolicyValidateAuthen, TreasuryValidatorValidateTreasury, FactoryValidatorValidateFactory, OrderValidatorValidateOrder, OrderValidatorValidateOrderSpending, } from "../plutus";
...
type StakeCredential =
  ...
  { Pointer: { slotNumber: bigint; transactionIndex: bigint; certificateIndex: bigint; } };
...
export function collectValidators(lucid: Translucent, seedTxIn: OutputReference) {
  ...
  const stakeCredential: StakeCredential = validatorHash2StakeCredential(orderSpendingValidatorHash);
  ...
  const factoryValidator = new FactoryValidatorValidateFactory(authenValidatorHash, treasuryValidatorHash, orderValidatorHash);
