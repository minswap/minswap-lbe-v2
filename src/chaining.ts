...
export async function doChaining(options: Chaining) {
  ...
  while (true) {
    ...
    let v = C.Transaction.from_bytes(T.fromHex(tx.toString())).to_js_value();

    console.log(
      v.body.outputs.map((o) => ({ addr: o.address, coin: o.amount })),
    );
    await submitTx(tx.toString());
    let transactionHash = tx.toHash();
    let txHash = C.TransactionHash.from_hex(transactionHash);
    logger.info(`chaining a ${transactionHash}`);
